.class Landroidx/slidingpanelayout/widget/SlidingPaneLayout$dispirit;
.super Landroidx/core/view/poolside;
.source "SlidingPaneLayout.java"


# annotations
.annotation system Ldalvik/annotation/EnclosingClass;
    value = Landroidx/slidingpanelayout/widget/SlidingPaneLayout;
.end annotation

.annotation system Ldalvik/annotation/InnerClass;
    accessFlags = 0x0
    name = "dispirit"
.end annotation


# instance fields
.field private final centurion:Landroid/graphics/Rect;

.field final synthetic tori:Landroidx/slidingpanelayout/widget/SlidingPaneLayout;


# direct methods
.method constructor <init>(Landroidx/slidingpanelayout/widget/SlidingPaneLayout;)V
    .locals 0

    .line 1
    iput-object p1, p0, Landroidx/slidingpanelayout/widget/SlidingPaneLayout$dispirit;->tori:Landroidx/slidingpanelayout/widget/SlidingPaneLayout;

    invoke-direct {p0}, Landroidx/core/view/poolside;-><init>()V

    .line 2
    new-instance p1, Landroid/graphics/Rect;

    invoke-direct {p1}, Landroid/graphics/Rect;-><init>()V

    iput-object p1, p0, Landroidx/slidingpanelayout/widget/SlidingPaneLayout$dispirit;->centurion:Landroid/graphics/Rect;

    return-void
.end method

.method private flocky(Landroidx/core/view/accessibility/centurion;Landroidx/core/view/accessibility/centurion;)V
    .locals 1

    .line 1
    iget-object v0, p0, Landroidx/slidingpanelayout/widget/SlidingPaneLayout$dispirit;->centurion:Landroid/graphics/Rect;

    .line 2
    invoke-virtual {p2, v0}, Landroidx/core/view/accessibility/centurion;->dismission(Landroid/graphics/Rect;)V

    .line 3
    invoke-virtual {p1, v0}, Landroidx/core/view/accessibility/centurion;->fermi(Landroid/graphics/Rect;)V

    .line 4
    invoke-virtual {p2}, Landroidx/core/view/accessibility/centurion;->manful()Z

    move-result v0

    invoke-virtual {p1, v0}, Landroidx/core/view/accessibility/centurion;->chimb(Z)V

    .line 5
    invoke-virtual {p2}, Landroidx/core/view/accessibility/centurion;->uppiled()Ljava/lang/CharSequence;

    move-result-object v0

    invoke-virtual {p1, v0}, Landroidx/core/view/accessibility/centurion;->unrounded(Ljava/lang/CharSequence;)V

    .line 6
    invoke-virtual {p2}, Landroidx/core/view/accessibility/centurion;->fruitive()Ljava/lang/CharSequence;

    move-result-object v0

    invoke-virtual {p1, v0}, Landroidx/core/view/accessibility/centurion;->lapidification(Ljava/lang/CharSequence;)V

    .line 7
    invoke-virtual {p2}, Landroidx/core/view/accessibility/centurion;->orthograph()Ljava/lang/CharSequence;

    move-result-object v0

    invoke-virtual {p1, v0}, Landroidx/core/view/accessibility/centurion;->limonene(Ljava/lang/CharSequence;)V

    .line 8
    invoke-virtual {p2}, Landroidx/core/view/accessibility/centurion;->camisade()Z

    move-result v0

    invoke-virtual {p1, v0}, Landroidx/core/view/accessibility/centurion;->fletcherism(Z)V

    .line 9
    invoke-virtual {p2}, Landroidx/core/view/accessibility/centurion;->deluge()Z

    move-result v0

    invoke-virtual {p1, v0}, Landroidx/core/view/accessibility/centurion;->infundibuliform(Z)V

    .line 10
    invoke-virtual {p2}, Landroidx/core/view/accessibility/centurion;->analcite()Z

    move-result v0

    invoke-virtual {p1, v0}, Landroidx/core/view/accessibility/centurion;->professionless(Z)V

    .line 11
    invoke-virtual {p2}, Landroidx/core/view/accessibility/centurion;->seroot()Z

    move-result v0

    invoke-virtual {p1, v0}, Landroidx/core/view/accessibility/centurion;->electrologist(Z)V

    .line 12
    invoke-virtual {p2}, Landroidx/core/view/accessibility/centurion;->heroise()Z

    move-result v0

    invoke-virtual {p1, v0}, Landroidx/core/view/accessibility/centurion;->druggery(Z)V

    .line 13
    invoke-virtual {p2}, Landroidx/core/view/accessibility/centurion;->unsuited()Z

    move-result v0

    invoke-virtual {p1, v0}, Landroidx/core/view/accessibility/centurion;->esculent(Z)V

    .line 14
    invoke-virtual {p2}, Landroidx/core/view/accessibility/centurion;->evaluative()Z

    move-result v0

    invoke-virtual {p1, v0}, Landroidx/core/view/accessibility/centurion;->endometrial(Z)V

    .line 15
    invoke-virtual {p2}, Landroidx/core/view/accessibility/centurion;->cryotherapy()I

    move-result v0

    invoke-virtual {p1, v0}, Landroidx/core/view/accessibility/centurion;->poolside(I)V

    .line 16
    invoke-virtual {p2}, Landroidx/core/view/accessibility/centurion;->nutant()I

    move-result p2

    invoke-virtual {p1, p2}, Landroidx/core/view/accessibility/centurion;->neutrally(I)V

    return-void
.end method


# virtual methods
.method public ceilometer(Landroid/view/View;Landroidx/core/view/accessibility/centurion;)V
    .locals 3

    .line 1
    invoke-static {p2}, Landroidx/core/view/accessibility/centurion;->anemoscope(Landroidx/core/view/accessibility/centurion;)Landroidx/core/view/accessibility/centurion;

    move-result-object v0

    .line 2
    invoke-super {p0, p1, v0}, Landroidx/core/view/poolside;->ceilometer(Landroid/view/View;Landroidx/core/view/accessibility/centurion;)V

    .line 3
    invoke-direct {p0, p2, v0}, Landroidx/slidingpanelayout/widget/SlidingPaneLayout$dispirit;->flocky(Landroidx/core/view/accessibility/centurion;Landroidx/core/view/accessibility/centurion;)V

    .line 4
    invoke-virtual {v0}, Landroidx/core/view/accessibility/centurion;->versailles()V

    const-string v0, "androidx.slidingpanelayout.widget.SlidingPaneLayout"

    .line 5
    invoke-virtual {p2, v0}, Landroidx/core/view/accessibility/centurion;->lapidification(Ljava/lang/CharSequence;)V

    .line 6
    invoke-virtual {p2, p1}, Landroidx/core/view/accessibility/centurion;->plodder(Landroid/view/View;)V

    .line 7
    invoke-static {p1}, Landroidx/core/view/ViewCompat;->getParentForAccessibility(Landroid/view/View;)Landroid/view/ViewParent;

    move-result-object p1

    .line 8
    instance-of v0, p1, Landroid/view/View;

    if-eqz v0, :cond_0

    .line 9
    check-cast p1, Landroid/view/View;

    invoke-virtual {p2, p1}, Landroidx/core/view/accessibility/centurion;->strobila(Landroid/view/View;)V

    .line 10
    :cond_0
    iget-object p1, p0, Landroidx/slidingpanelayout/widget/SlidingPaneLayout$dispirit;->tori:Landroidx/slidingpanelayout/widget/SlidingPaneLayout;

    invoke-virtual {p1}, Landroid/view/ViewGroup;->getChildCount()I

    move-result p1

    const/4 v0, 0x0

    :goto_0
    if-ge v0, p1, :cond_2

    .line 11
    iget-object v1, p0, Landroidx/slidingpanelayout/widget/SlidingPaneLayout$dispirit;->tori:Landroidx/slidingpanelayout/widget/SlidingPaneLayout;

    invoke-virtual {v1, v0}, Landroid/view/ViewGroup;->getChildAt(I)Landroid/view/View;

    move-result-object v1

    .line 12
    invoke-virtual {p0, v1}, Landroidx/slidingpanelayout/widget/SlidingPaneLayout$dispirit;->phagocyte(Landroid/view/View;)Z

    move-result v2

    if-nez v2, :cond_1

    invoke-virtual {v1}, Landroid/view/View;->getVisibility()I

    move-result v2

    if-nez v2, :cond_1

    const/4 v2, 0x1

    .line 13
    invoke-static {v1, v2}, Landroidx/core/view/ViewCompat;->setImportantForAccessibility(Landroid/view/View;I)V

    .line 14
    invoke-virtual {p2, v1}, Landroidx/core/view/accessibility/centurion;->stylolite(Landroid/view/View;)V

    :cond_1
    add-int/lit8 v0, v0, 0x1

    goto :goto_0

    :cond_2
    return-void
.end method

.method public deprecate(Landroid/view/View;Landroid/view/accessibility/AccessibilityEvent;)V
    .locals 0

    .line 1
    invoke-super {p0, p1, p2}, Landroidx/core/view/poolside;->deprecate(Landroid/view/View;Landroid/view/accessibility/AccessibilityEvent;)V

    const-string p1, "androidx.slidingpanelayout.widget.SlidingPaneLayout"

    .line 2
    invoke-virtual {p2, p1}, Landroid/view/accessibility/AccessibilityEvent;->setClassName(Ljava/lang/CharSequence;)V

    return-void
.end method

.method public phagocyte(Landroid/view/View;)Z
    .locals 1

    .line 1
    iget-object v0, p0, Landroidx/slidingpanelayout/widget/SlidingPaneLayout$dispirit;->tori:Landroidx/slidingpanelayout/widget/SlidingPaneLayout;

    invoke-virtual {v0, p1}, Landroidx/slidingpanelayout/widget/SlidingPaneLayout;->expiry(Landroid/view/View;)Z

    move-result p1

    return p1
.end method

.method public vidar(Landroid/view/ViewGroup;Landroid/view/View;Landroid/view/accessibility/AccessibilityEvent;)Z
    .locals 1

    .line 1
    invoke-virtual {p0, p2}, Landroidx/slidingpanelayout/widget/SlidingPaneLayout$dispirit;->phagocyte(Landroid/view/View;)Z

    move-result v0

    if-nez v0, :cond_0

    .line 2
    invoke-super {p0, p1, p2, p3}, Landroidx/core/view/poolside;->vidar(Landroid/view/ViewGroup;Landroid/view/View;Landroid/view/accessibility/AccessibilityEvent;)Z

    move-result p1

    return p1

    :cond_0
    const/4 p1, 0x0

    return p1
.end method
